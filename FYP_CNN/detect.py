...
def run(
    ...
    f0 = open(cfp + '/CNN/boundingBoxes/demo_img.txt', 'r')
    line = f0.readline()
    i = 1
    while line != '':
    ...
    for path, im, im0s, vid_cap, s in dataset:
        with dt[0]:
            ...
            for key in BoundingBoxSet:
                ...
                img_current = Image.fromarray(img, 'RGB')
                img_current = img_current.crop(((int(boxCoordinates[0])), int(boxCoordinates[1]), int(boxCoordinates[2]), int(boxCoordinates[3])))
